...
class PostgresClient:
    ...
    def get_all_eam_positions(self) -> list[EAMPosition]:
        """
        Retrieve all EAM devices from the database.
        """
        return self._get_all(
            EAMPositionORM, EAMPosition.from_orm, "Error querying EAM positions"
        )
        ...
        with self.Session() as session:
            with session.begin():
                ...
                for device, changes in to_update:
                    # device is a Pydantic EAMDevice/EAMPosition -> use equipment_no
                    orm_obj = session.get(orm_cls, device.equipment_no)
                    if orm_obj:
    ...
    def sync_eam_positions(
        self,
        to_insert: list[EAMPosition],
        to_update: list[tuple[EAMPosition, dict[str, Any]]],
        to_delete: list[str],
    ) -> None:
        """
        Sync EAM Positions by delegating to the generic sync implementation.
        """
        self._sync_devices(
            EAMPositionORM,
