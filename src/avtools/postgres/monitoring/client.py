...
class PostgresMonitoringClient:
    ...
    def __init__(self, postgres_url: str) -> None:
        ...
        try:
            ...
            raise PostgresMonitoringClientError(
                "Failed to create monitoring tables"
            ) from exc

    def upsert_projector_monitoring(
        self, records: Iterable[ProjectorMonitoring]
    ) -> int:
        ...
        with self._session_maker() as session:
            try:
                ...
                raise PostgresMonitoringClientError(
                    "Failed to upsert monitoring rows"
                ) from exc

        logger.info("postgres_monitoring_projector_upserted", rows=len(rows))
        return len(rows)

    def upsert_device_sysdescr_monitoring(
        self, records: Iterable[DeviceSysDescrMonitoring]
    ) -> int:
        ...
        with self._session_maker() as session:
            try:
                ...
                session.rollback()
                logger.exception(
                    "postgres_monitoring_sysdescr_upsert_failed", rows=len(rows)
                )
                raise PostgresMonitoringClientError(
